			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="row mb-4">
							<div class="col-4">
								<label for="add_department_title" class="form-label text-uppercase fw-bold me-3">Title <span class="mandatory">*</span></label>
								...
								<div>
									...
									<template v-for="error of v$.departmentForAdd.title.$errors" :key="error.$uid">
					...
					<div class="modal-body">
						<div class="row mb-4">
							<div class="col-4">
								...
								<div>
									<span v-if="readDepartment.title">{{ readDepartment.title }}</span
							...
							<div class="col-4">
								...
								<div>
									<span v-if="readDepartment.creator?.name">{{ readDepartment.creator?.name }}</span
									><span v-else><i>Not specified</i></span>
								</div>
							</div>
						</div>
	...
	export default {
		...
		data() {
			return {
				dataprops: {
					id: "department-list",
					class: "a",
					base_url: this.docRoot + "/api/department/",
					columns: [
		...
		methods: {
				...
				axios
					...
					.then(async function (response) {
						...
						if (status == 1) {
							...
							thisVar.v$.$reset();
						} else thisVar.showErrors("Department could not be saved successfully", response.data.messages, "bottom", 3000);
					})
			...
			prepareEditDepartment(department) {
				...
				if (this.departmentForAdd.is_percentage == 1) this.departmentForAdd.is_percentage = true;
				else this.departmentForAdd.is_percentage = false;
