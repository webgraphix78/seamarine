<template>
	<div class="row mb-1 d-flex align-items-center">
		...
		<!-- <div class="col-sm-3 d-none d-sm-block text-end">
						...
						<tr v-for="row of allRows" :key="row.id">
							<template v-for="(rowValue, index) in row.rowValues" :key="rowValue.id">
								<td :class="rowValue.class + ' ' + (dataprops.columns[index]?.align == 'center' ? 'text-center' : (dataprops.columns[index]?.align == 'right' ? 'text-end' : ''))">
									...
									<div v-else v-html="(rowValue.prefix?rowValue.prefix:'')+rowValue.value+(rowValue.suffix?rowValue.suffix:'')"></div>
								</td>
							</template>
							...
							<td class="text-center" style="width: 1px; white-space: nowrap;padding-left: 20px;padding-right: 20px;">
								<div class="d-flex flex-row gap-2 justify-content-center">
									<a class="btn btn-sm" :class="action.class" href="#" role="button" v-for="(action, key) of row.rowObject.actions" @click="$emit(action.action, row.rowObject, action.additional_params?.join(','))" v-html="action.title"></a>
								</div>
							</td>
						</tr>
					</template>
