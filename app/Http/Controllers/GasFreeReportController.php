	...
	public function get(Request $request){
		...
		if ($searchType == "simple") {
			if (isset($input["q"]) && strlen(trim($input["q"])) > 0) {
				$gasfreereportList = $gasfreereportList->where(function ($query) use ($input) {
				...
				$gasfreereportList = $gasfreereportList->orWhereHas('rel_customer_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
				$gasfreereportList = $gasfreereportList->orWhereHas('rel_surveyor_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
			}
		}
