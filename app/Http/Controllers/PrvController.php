	...
	public function get(Request $request){
		...
		if ($searchType == "simple") {
			if (isset($input["q"]) && strlen(trim($input["q"])) > 0) {
				$prvList = $prvList->where(function ($query) use ($input) {
				...
				$prvList = $prvList->orWhereHas('rel_customer_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
				$prvList = $prvList->orWhereHas('rel_surveyor_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
			}
		}
