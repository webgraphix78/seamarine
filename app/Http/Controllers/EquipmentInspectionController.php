	...
	public function get(Request $request){
		...
		if ($searchType == "simple") {
			if (isset($input["q"]) && strlen(trim($input["q"])) > 0) {
				$equipmentinspectionList = $equipmentinspectionList->where(function ($query) use ($input) {
				...
				$equipmentinspectionList = $equipmentinspectionList->orWhereHas('rel_customer_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
				$equipmentinspectionList = $equipmentinspectionList->orWhereHas('rel_surveyor_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
			}
		}
