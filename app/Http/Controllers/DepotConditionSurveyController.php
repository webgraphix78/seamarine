	...
	public function get(Request $request){
		...
		if ($searchType == "simple") {
			if (isset($input["q"]) && strlen(trim($input["q"])) > 0) {
				$depotconditionsurveyList = $depotconditionsurveyList->where(function ($query) use ($input) {
				...
				$depotconditionsurveyList = $depotconditionsurveyList->orWhereHas('rel_customer_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
				$depotconditionsurveyList = $depotconditionsurveyList->orWhereHas('rel_surveyor_id', function ($query) use ($input) {
					$query = $query->where('name', 'like', '%' . trim($input['q']) . '%');
				});
			}
		}
