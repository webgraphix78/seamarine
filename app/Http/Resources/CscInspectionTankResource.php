...
class CscInspectionTankResource extends JsonResource
{
    /**
    ...
    public function toArray(Request $request): array{
		...
		if (isset($input['current_user_id']) && $input['current_user_id'] > 0) {
			...
			$actions = ActionsService::generateActions(class_basename(CscInspectionTank::class), $currentUser->role_id, $this->status);
		};
		return [
			'id' => $this->id,
			...
			'insp_perfom_steam_pressure_na' => $this->insp_perfom_steam_pressure_na,
			'insp_perfom_steam_pressure_in' => $this->insp_perfom_steam_pressure_in,
			'insp_perfom_steam_pressure_remark' => $this->insp_perfom_steam_pressure_remark,
